...
fn parse_input(input: &str) -> (Vec<usize>, Duration) {
    ...
    let seat_indices = input
        ...
        .enumerate()
        .filter(|(_, s)| s == &&b'L')
        .map(|(i, _)| i - (i / (unsafe { COL } as usize + 1)))
        .collect();
    let end = Instant::now();
...
fn solve_part1(seat_indices: &[usize]) -> (usize, Duration) {
    ...
    let mut cur = vec![Seat::Floor; (unsafe { ROW * COL }) as usize];
    ...
    let seats: Vec<(usize, Vec<usize>)> = seat_indices
        ...
        .map(|&i| (i, (0..9)
            .filter(|&r| r != 4)
            .map(|r| ((i as isize % unsafe {COL}) + r % 3 - 1, (i as isize / unsafe {COL}) + r / 3 - 1))
            .filter(|(x, y)| *x >= 0 && *y >= 0 && *x < unsafe {COL} && *y < unsafe {ROW})
            .map(|(x, y)| (y * unsafe {COL} + x) as usize)
...
fn solve_part2(seat_indices: &[usize]) -> (usize, Duration) {
    ...
    let seats: Vec<(usize, Vec<usize>)> = seat_indices
        ...
        .map(|&i| (i, (0..9)
            ...
            .map(|r| (r % 3 - 1, r / 3 - 1))
            .filter_map(|(rx, ry)| (1..)
                .map(|f| ((i as isize % unsafe {COL}) + rx * f, (i as isize / unsafe {COL}) + ry * f))
                .take_while(|(x, y)| *x >= 0 && *y >= 0 && *x < unsafe {COL} && *y < unsafe {ROW})
                .map(|(x, y)| (y * unsafe {COL} + x) as usize)
