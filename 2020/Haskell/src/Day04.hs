...
requiredFields =
    map Text.pack ["byr", "iyr", "eyr", "hgt", "hcl", "ecl", "pid"]

validatePassport :: [String] -> Bool
validatePassport ("hgt" : value : _)
    | unit == "cm" = not $ height < 150 || height > 193
...
validatePassport (key : value : _)
    ...
    | key == "hcl" = (length value == 7) && (head value == '#') && all
...
day04 = do
    input_Text <- Text.lines
        <$> Text.readFile "/home/jamie/Git/AdventOfCode/2020/Inputs/day_04.txt"
    let input_strs = map Text.unpack input_Text
    -- part 1
    let filtered = filter
            (\passport -> all
                (\need -> Text.isInfixOf need (Text.pack passport))
                requiredFields
            )
