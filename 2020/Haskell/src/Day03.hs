...
isTree map x y = ((map !! y) !! x_mod) == '#'
    where x_mod = x `mod` length (head map)

getTreesSlopeRecursive :: Map -> Int -> Int -> Int -> Int -> Int -> Int
getTreesSlopeRecursive worldMap xStep yStep x y acc
    | y >= yLimit = acc
    | isTree worldMap x y = getTreesSlopeRecursive worldMap
                                                   xStep
                                                   yStep
                                                   (x + xStep)
                                                   (y + yStep)
                                                   (acc + 1)
    | otherwise = getTreesSlopeRecursive worldMap
                                         xStep
                                         yStep
                                         (x + xStep)
                                         (y + yStep)
...
getTreesSlope worldMap xStep yStep =
    getTreesSlopeRecursive worldMap xStep yStep 0 0 0

day03 :: IO ()
day03 = do
    input_Text <- Text.lines
        <$> Text.readFile "/home/jamie/Git/AdventOfCode/2020/Inputs/day_03.txt"
